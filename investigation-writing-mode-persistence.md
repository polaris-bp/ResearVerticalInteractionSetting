...
| ケース | インタラクション設定 | Widget初期化状態 | アイテム変更時の動作 |
|--------|-------------------|-----------------|-------------------|
| A | なし（継承） | 未初期化 | CSS継承で自動追従 |
| B | なし（継承） | 初期化済み | CSS継承で自動追従 |
| C | あり（明示） | 未初期化 | **変更されない**（クラス維持） |
| D | あり（明示） | 初期化済み | **リセットされる**（クラス削除） |

### 重要な発見: initCreatorのイベントリスナーは常にアクティブ

`initCreator`で登録されたイベントリスナーは、**Widgetが初期化された後は常にアクティブ**です。
インタラクションが選択されていなくても、`item-writing-mode-changed`イベントは受信されます。

```javascript
// Widget.js の initCreator - 一度呼ばれると常にアクティブ
$itemBody.on('item-writing-mode-changed', () => {
    this.element.removeClass(verticalWriting.WRITING_MODE_VERTICAL_RL_CLASS);
    this.element.removeClass(verticalWriting.WRITING_MODE_HORIZONTAL_TB_CLASS);
    // ...
...
### ケースCの詳細（Widget未初期化時のみ発生）

**ケースCは、Widgetがまだ初期化されていない場合にのみ発生します。**

#### ケースCの再現手順

```
1. アイテムを作成
2. choiceInteractionを追加、縦書きに設定
3. アイテムを保存して閉じる
4. アイテムを再度開く（この時点でWidgetはまだ初期化されていない）
5. インタラクションを選択せずに、アイテムのwriting-modeを変更
6. Widgetが未初期化なのでイベントリスナーがなく、クラスが維持される
```
...
初期状態:
  ...
  Widget: 初期化済み（インタラクションを一度でも選択した後）

操作1: アイテムを縦書きに変更
├→ イベント 'item-writing-mode-changed' が発火
├→ インタラクションの 'writing-mode-vertical-rl' クラスが削除される
├→ インタラクションはクラスなし → アイテムの設定を継承
└→ 表示: 両方縦書き（見た目は変わらない）
...
2. **PCI個別設定がある + Widget初期化済み** → リセットされてアイテム設定を継承
3. **PCI個別設定がある + Widget未初期化** → 設定維持（クラスが残る）

```
アイテム変更時:
  ├─ PCI個別設定なし → 自動的にアイテム設定に追従（CSS継承）
  ├─ PCI個別設定あり + Widget初期化済み → リセット → アイテム設定を継承
  └─ PCI個別設定あり + Widget未初期化 → 変更なし（PCI設定を維持）
```

**注意:** 「Widget初期化済み」とは、アイテムを開いた後にインタラクションを一度でも選択した状態を指します。
一度初期化されると、インタラクションが選択されていなくてもイベントリスナーは動作し続けます。
